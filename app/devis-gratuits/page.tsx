...
import { FormState, INITIAL_FORM_STATE, PricingByFormule } from '@/lib/form-types';
...
function InventaireIAPageInner() {
  ...
  const [pricing, setPricing] = useState<PricingByFormule | null>(null);
  ...
  useEffect(() => {
    if (formState.surfaceM2) {
      ...
      // Calculer un PricingResult par formule, selon la grille La Poste (€/m³ par bande de distance)
      const eco = calculatePricing(
        formState.surfaceM2,
      ...
      const premium = calculatePricing(
        formState.surfaceM2,
        formState.housingType,
        formState.density,
        distance,
  ...
  const saveToBackend = useCallback(async (state: FormState) => {
    ...
    try {
      ...
      const currentPricing = pricing ? pricing[state.formule] : undefined;
      ...
      await updateLead(state.leadId, {
        ...
        estimatedVolume: currentPricing?.volumeM3,
        density: mapDensityToBackend(state.density),  // ⚠️ Mapping: 'normal' → 'MEDIUM'
        
        // Formule & Prix
        formule: state.formule,
        estimatedPriceMin: currentPricing?.prixMin,
        estimatedPriceAvg: currentPricing?.prixAvg,
        estimatedPriceMax: currentPricing?.prixMax,
              ...
              <div className="mb-6">
                ...
                {pricing && (
                  <p className="text-[#4b5c6b] mb-4">
                    ...
                    {' '}:{' '}
                    <span className="font-bold text-[#6BCFCF] text-xl">
                      {pricing[formState.formule].volumeM3} m³
                    </span>
                  </p>
                )}
                ...
                {pricing && (
                    <>
                      ...
                      prixMin={pricing.ECONOMIQUE.prixMin}
                      prixAvg={pricing.ECONOMIQUE.prixAvg}
                      prixMax={pricing.ECONOMIQUE.prixMax}
                      ...
                      prixMin={pricing.STANDARD.prixMin}
                      prixAvg={pricing.STANDARD.prixAvg}
                      prixMax={pricing.STANDARD.prixMax}
                        recommended
                      ...
                      prixMin={pricing.PREMIUM.prixMin}
                      prixAvg={pricing.PREMIUM.prixAvg}
                      prixMax={pricing.PREMIUM.prixMax}
